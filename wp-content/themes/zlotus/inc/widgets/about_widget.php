...
add_action( 'widgets_init', 'zlotus_about_load_widget' );

function zlotus_about_load_widget() {
	register_widget( 'zlotus_about_widget' );
}

class zlotus_about_widget extends WP_Widget {
	...
	function __construct() {
		/* Widget settings. */
		$widget_ops = array( 'classname' => 'zlotus_about_widget', 'description' => __('An About Me Widget', 'zlotus') );

		/* Widget control settings. */
		$control_ops = array( 'width' => 250, 'height' => 350, 'id_base' => 'zlotus_about_widget' );

		/* Create the widget. */
		parent::__construct( 'zlotus_about_widget', __('ZLOTUS: About Me', 'zlotus'), $widget_ops, $control_ops );
	}
	...
	function form( $instance ) {
		...
		<!-- image url -->
		<p>
			<label for="<?php echo $this->get_field_id( 'image' ); ?>"><?php esc_html_e( 'Image URL', 'zlotus' ); ?>:</label>
			...
			<small><?php esc_html_e( 'Enter the image URL you want to use. You can upload your image via Media > Add New', 'zlotus' ); ?></small>
		</p>
		
		<!-- image link -->
		<p>
			<label for="<?php echo $this->get_field_id( 'image_link' ); ?>"><?php esc_html_e( 'Image Link', 'zlotus' ); ?>:</label>
			...
			<small><?php esc_html_e( 'Enter a link you want the about me image to go to.', 'zlotus' ); ?></small>
		</p>
		
		<!-- description -->
		<p>
			<label for="<?php echo $this->get_field_id( 'description' ); ?>"><?php esc_html_e( 'About me text', 'zlotus' ); ?>:</label>
		...
		<!-- autograph url -->
		<p>
			<label for="<?php echo $this->get_field_id( 'signing' ); ?>"><?php esc_html_e( 'Autograph Image URL', 'zlotus' ); ?>:</label>
