...
add_action( 'widgets_init', 'zlotus_promo_load_widget' );

function zlotus_promo_load_widget() {
	register_widget( 'zlotus_promo_widget' );
}

class zlotus_promo_widget extends WP_Widget {
	...
	function __construct() {
		/* Widget settings. */
		$widget_ops = array( 'classname' => 'zlotus_promo_widget', 'description' => __('Display a promo box with an image and link', 'zlotus') );

		/* Widget control settings. */
		$control_ops = array( 'width' => 250, 'height' => 350, 'id_base' => 'zlotus_promo_widget' );

		/* Create the widget. */
		parent::__construct( 'zlotus_promo_widget', __('ZLOTUS: Promo Box', 'zlotus'), $widget_ops, $control_ops );
	}
	...
	function form( $instance ) {
		...
		$defaults = array( 'title' => '', 'image_url' => '', 'image_link' => '', 'heading' => '', 'border' => false, 'height' => 130, 'margin' => 42);
		...
		<!-- Background URL -->
		<p>
			<label for="<?php echo $this->get_field_id( 'image_url' ); ?>"><?php esc_html_e( 'Background Image URL', 'zlotus' ); ?>:</label>
			...
			<small><?php esc_html_e( 'Enter the background image URL you want to use. You can upload your background image via Media > Add New', 'zlotus' ); ?></small>
		</p>
		
		<!-- Image Text -->
		<p>
			<label for="<?php echo $this->get_field_id( 'heading' ); ?>"><?php esc_html_e( 'Promo Text', 'zlotus' ); ?>:</label>
		...
		<!-- Link -->
		<p>
			<label for="<?php echo $this->get_field_id( 'image_link' ); ?>"><?php esc_html_e( 'Promo Link', 'zlotus' ); ?>:</label>
			...
			<small><?php esc_html_e( 'Enter a link you want the promo box to go to.', 'zlotus' ); ?></small>
		</p>
		
		<!-- Height -->
		<p>
			<label for="<?php echo $this->get_field_id( 'height' ); ?>"><?php esc_html_e( 'Promo Box Height', 'zlotus' ); ?>:</label><br>
		...
		<!-- Margin -->
		<p>
			<label for="<?php echo $this->get_field_id( 'margin' ); ?>"><?php esc_html_e( 'Promo Box Margin Bottom', 'zlotus' ); ?>:</label><br>
		...
		<!-- New tab -->
		<p>
			<label for="<?php echo $this->get_field_id( 'newtab' ); ?>"><?php esc_html_e( 'Open link in a new tab?', 'zlotus' ); ?></label>
		...
		<!-- Border -->
		<p>
			<label for="<?php echo $this->get_field_id( 'border' ); ?>"><?php esc_html_e( 'Hide Separator Line Above Widget?', 'zlotus' ); ?></label>
